...
The BTF pattern-break cases make the failure more concrete: the model can possess both the recurring source pattern and evidence of the target-side boundary, yet still fail to make the difference change its forecast. This suggests measuring `boundary response` directly rather than treating difference awareness as sufficient.
